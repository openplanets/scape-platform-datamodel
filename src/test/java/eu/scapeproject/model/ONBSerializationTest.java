...
public class ONBSerializationTest {
	...
	@Test
	public void testONBGBSSerialization() throws Exception {
		IntellectualEntity.Builder e = new IntellectualEntity.Builder();
		...
		// create a dummy google book scan technical metadata
		ProductionNotes gbs = new ProductionNotes.Builder()
				.badPages("1,4")
				.missingPages("2")
				.tightBoundPages("")
				.build();
	...
	public void testONBGoogleBookScanDeserialization() throws Exception {
		...
		assertTrue(e.getRepresentations().get(0).getTechnical() instanceof ProductionNotes);
		assertNotNull(e.getRepresentations().get(0).getTitle());
		ProductionNotes notes = (ProductionNotes) e.getRepresentations().get(0).getTechnical();
		assertTrue(notes.getBadPages().equals("35.61"));
	}

	@Test
	public void testONBProductionNotesDeserialization() throws Exception {
		Object o = SCAPEMarshaller.getInstance().getJaxbUnmarshaller()
				.unmarshal(this.getClass().getClassLoader().getResourceAsStream("ONB_production_notes.xml"));
		assertTrue("Object is of wrong type " + o.getClass().getName(), o instanceof ProductionNotes);
	}
