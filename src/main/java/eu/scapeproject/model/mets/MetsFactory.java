...
public class MetsFactory {
    ...
    public IntellectualEntity toIntellectualEntity(final MetsDocument doc) throws Exception{
        final DescriptiveMetadata dmd;
        if (doc.getDmdSec().getMetadataWrapper().getMdType().equals("DC" )){
            dmd = (DCMetadata) unmarshaller.unmarshal(new StringReader((String) doc.getDmdSec().getMetadataWrapper().getXmlData()));
        }else{
            dmd=null;
        }
        final IntellectualEntity entity=new IntellectualEntity.Builder()
